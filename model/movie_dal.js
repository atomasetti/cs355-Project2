...
exports.Insert = function(movie_info, callback) {
     ...
     */
    var dynamic_query = 'INSERT INTO Movies (title, tagline) VALUES (' +
        '\'' + movie_info.title + '\', ' +
        '\'' + movie_info.tagline + '\' ' +
        ');';
...
exports.Update = function(movie_id, title, tagline, genre_ids, callback) {
    ...
    connection.query('UPDATE Movies SET title = ?, tagline = ? WHERE movieID = ?', values,
        function(err, result){
            if(err) {
...
//function Delete(movie_id, callback) {
    var qry = 'DELETE FROM Movies WHERE movieID = ?';
